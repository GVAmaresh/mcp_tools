import httpx
from .errors import APIError
from .logger import log
from config import settings 

class HTTPClient:
    def __init__(self, base_url: str, default_params: dict | None = None):
        self.base_url = base_url
        self._client = httpx.AsyncClient(base_url=self.base_url, timeout=10.0)
        self.default_params = default_params or {}

    async def get_json(self, endpoint: str, params: dict | None = None) -> dict:
        merged_params = {**self.default_params, **(params or {})}
        try:
            response = await self._client.get(endpoint, params=merged_params)
            response.raise_for_status()
            return response.json()
        except httpx.HTTPStatusError as e:
            log.error(f"HTTP error: {e.response.status_code} - {e.response.text}")
            raise APIError(f"API request failed with status {e.response.status_code}")
        except httpx.RequestError as e:
            log.error(f"Request error: {e}")
            raise APIError(f"Error requesting {e.request.url}")

    async def close(self):
        await self._client.aclose()


geocoding_client = HTTPClient(base_url="https://geocoding-api.open-meteo.com")
weather_client = HTTPClient(base_url="https://api.open-meteo.com")
google_api_client = HTTPClient(
    base_url="https://maps.googleapis.com/maps/api/place",
    default_params={"key": settings.GOOGLE_MAPS_API_KEY}
)
